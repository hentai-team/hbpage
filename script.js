...
App.setup = function() {
  ...
  this.canvas = canvas;
  document.getElementById('wallpaper').appendChild(canvas);
...
App.move = function() {
  for (var i = 0; i < this.particles.length; i++) {
    // Get particle
    var p = this.particles[i]; // исправлена опечатка pparticles -> particles
    ...
    if (Math.random() < 0.5) {
      ...
      if (!gridSpot.isEdge) {
        ...
        // Проверяем существование соседних точек
        var spots = [];
        if (this.grid[topIndex]) spots.push(this.grid[topIndex]);
        if (this.grid[bottomIndex]) spots.push(this.grid[bottomIndex]);
        if (this.grid[leftIndex]) spots.push(this.grid[leftIndex]);
        if (this.grid[rightIndex]) spots.push(this.grid[rightIndex]);
        
        // Выбираем соседа с максимальным значением поля
        if (spots.length > 0) {
          var chaos = 30;
          var maxFieldSpot = spots.reduce(function(max, spot) {
